...
IChatClient innerChatClient = new AzureOpenAIClient(new Uri(builder.Configuration["OpenAI:Uri"]!), new ApiKeyCredential(builder.Configuration["OpenAI:Key"]!))
...
builder.Services.AddEmbeddingGenerator<string, Embedding<float>>(pipeline => pipeline
    .Use(new OllamaEmbeddingGenerator(new Uri("http://127.0.0.1:11434"), modelId: "nomic-embed-text")));
